document.addEventListener('DOMContentLoaded', () => {
  ...
  const AUTOPLAY_MS = 3500;      // скорость автолистывания (мс)
  const RESUME_IDLE_MS = 6000;   // после действий через сколько снова включать (мс)
  let timer = null;
  ...
  const setActive = (n) => {
    ...
    if (dotsWrap) {
      const dots = dotsWrap.querySelectorAll('.about-dot');
  ...
  if (dotsWrap) {
    ...
    slides.forEach((_, i) => {
      ...
      b.type = 'button';
      b.className = 'about-dot' + (i === idx ? ' is-active' : '');
